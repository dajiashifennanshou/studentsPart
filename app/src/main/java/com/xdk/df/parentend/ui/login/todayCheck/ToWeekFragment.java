 ...
 */
public class ToWeekFragment extends BaseFragment {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1) {
                ...
                String a = listCheck.get(0).getHappendate();
                String b = listCheck.get(0).getHappentime();
                String c = listCheck.get(0).getChecktype();
                String d = listCheck.get(0).getHappenaddress();
                String dd = listCheck.get(0).getCheckaspect();
                String dg = listCheck.get(0).getHappenaddress();
                getTodayCheck();
            }
        }
    };
    ...
    private void getTodayCheck() {
        for (int i = 0; i < listCheck.size(); i++) {
            int vip = HttpHelper.compare_date(listCheck.get(i).getHappendate(), new SimpleDateFormat("yyyy-MM-dd").format(getCurrentYearStartTime()));
            if (vip == 0 || vip == 1) {
                int flag = HttpHelper.compare_date(listCheck.get(i).getHappendate(), new SimpleDateFormat("yyyy-MM-dd").format(getCurrentYearEndTime()));
                if (flag == -1) {
    ...
    public Date getCurrentYearEndTime() {
        ...
        try {
            ...
            now = new SimpleDateFormat("yyyy-MM-dd").parse(new SimpleDateFormat("yyyy-MM-dd").format(c.getTime()) + " 23:59:59");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return now;
